...
for i in range(len(courseCodes)):
    for x in range(8):
        # TODO: make the timestamp dynamic (https://timestamp.online/article/how-to-get-current-timestamp-in-python)
        # TODO: make the term dynamic from a commandline argument
        response = requests.get("https://scheduleme.wlu.ca/vsb/add_suggest.jsp?term=202105&cams=C_K_T_V_W_Z_CC_G_X_Y_MC&course_add=" +
                                courseCodes[i] + "&page_num=" + str(x) + "&_=1615483280")
